...
(function ($, document, window) {
    ...
    $.fn.weatherFeed = function (options) {
        ...
        this.each(function () {
            ...
            $.ajax({
                url: "https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20geo.places%20where%20text%3D%22" + city + "%22&format=json&diagnostics=true&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys&callback=?",
                dataType: "json",
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            for (var i = 0; i < data.query.results.channel.item.forecast.length && i < days; i++) {
                                switch(data.query.results.channel.item.forecast[i].text) {
                                    case "Mostly Cloudy":
                                        data.query.results.channel.item.forecast[i].text = "Bastante Nuboso";
                                        break;
                                    case "Mostly Sunny":
